...
def get_documents_from_CB(cb_object, primary_key, match_ids):
    ...
    try:
        # entire document
        query = f'select * from `{cb_object["bucket"]}`.`{cb_object["scope"]}`.`{cb_object["collection"]}` where meta().id in $1'
        result = cluster.query(query, QueryOptions(positional_parameters=[match_ids]))
        for row in result.rows():
            cb_document = row[cb_object["collection"]]
...
def diff_documents(incoming_document, cb_matched_document):
    return ujson.dumps(incoming_document, sort_keys=True) != ujson.dumps(
        cb_matched_document, sort_keys=True
    )
